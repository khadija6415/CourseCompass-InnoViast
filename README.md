...
**Change shipped in response:** Resource cards now show a matched/missing breakdown of each covered topic against the student's uploaded syllabus, visible without an extra click. Full testing notes are in `MVP_Testing_Results.md`.
...
- **Syllabus Upload and Match**: students paste their course outline or upload a PDF; the system extracts keywords and computes a personalized match percentage and topic-level breakdown for every resource in that topic, without any external AI API
...
(See the /screenshots folder for the full set: course selection, resource list with match tags, resource detail with the match gauge, syllabus upload panel with personalized results, and the admin dashboard.)
